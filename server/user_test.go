...
func (e eqCreateUserParamsMatcher) Matches(x interface{}) bool {
	...
	err := testutils.CheckPassword(e.password, arg.HashedPassword)
	if err != nil {
		return false
	}
...
func TestCreateUserAPI(t *testing.T) {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					CreateUser(gomock.Any(), gomock.Any()).
					Times(1).
					Return(repo.User{}, repo.ErrUniqueViolation)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			url := fmt.Sprintf("%s/users", generateRandomPort())
...
func TestLoginUserAPI(t *testing.T) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			url := fmt.Sprintf("%s/users/login", generateRandomPort())
			request, err := http.NewRequest(http.MethodPost, url, bytes.NewReader(data))
			require.NoError(t, err)
...
func randomUser(t *testing.T) (user repo.User, password string) {
	password = testutils.RandomString(6)
	hashedPassword, err := testutils.HashPassword(password)
	require.NoError(t, err)
